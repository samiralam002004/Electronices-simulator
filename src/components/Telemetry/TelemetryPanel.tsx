...
export const TelemetryPanel: React.FC<TelemetryPanelProps> = ({
  ...
  // Calculation Search & Category Filters State
  ...
  // Filter explanations based on selected category & search string
  const filteredExplanations = explanations.filter((step) => {
    const matchesCategory = calcCategoryFilter === "All" || step.category === calcCategoryFilter;
    const q = calcSearchQuery.toLowerCase();
    const matchesSearch =
      !q ||
      step.title.toLowerCase().includes(q) ||
      step.formula.toLowerCase().includes(q) ||
      step.description.toLowerCase().includes(q) ||
      step.result.toLowerCase().includes(q);
  ...
  const handleCopyCalculations = () => {
    const text = explanations
      .map(
        (s, idx) =>
          `[Step ${idx + 1}] ${s.title} (${s.category})\nDescription: ${s.description}\nFormula: ${s.formula}\nSubstituted: ${s.substitutedValues}\nResult: ${s.result}\n`
      )
      .join("\n----------------------------------------\n\n");
          ...
          {activeTab === "formulas" && (
            <div className="space-y-3 text-xs">
              ...
              <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-800 space-y-2.5">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-2">
                  <div className="flex items-center gap-1.5 text-emerald-400 font-bold text-xs">
                    <Calculator className="w-4 h-4" />
                    <span>Complete Derivations & Step-by-Step Calculations</span>
                    <span className="bg-emerald-950 text-emerald-400 text-[10px] px-2 py-0.5 rounded-full border border-emerald-800 font-mono">
                      {filteredExplanations.length} Steps
                    </span>
                  </div>

                  <button
                    onClick={handleCopyCalculations}
                    className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-[11px] font-semibold rounded-lg border border-slate-700 flex items-center gap-1 transition-all cursor-pointer"
                  >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <input
                    type="text"
                    placeholder="Search calculation, formula, law, component..."
                    value={calcSearchQuery}
                    onChange={(e) => setCalcSearchQuery(e.target.value)}
                    className="bg-slate-900 border border-slate-800 text-slate-200 text-xs rounded-lg px-2.5 py-1 font-mono focus:outline-none focus:border-emerald-500 min-w-[200px] shrink-0"
                  />
              ...
              {filteredExplanations.length === 0 ? (
                <div className="text-slate-400 text-center py-6 bg-slate-950/60 rounded-xl border border-slate-800">
                  <Calculator className="w-8 h-8 text-slate-600 mx-auto mb-2" />
                  <p className="font-semibold">No calculations match filter "{calcSearchQuery || calcCategoryFilter}".</p>
                  <p className="text-[11px] text-slate-500 mt-1">Add loads or sources to canvas to generate step-by-step mathematical steps.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {filteredExplanations.map((step, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2 hover:border-slate-700 transition-all"
                    >
                      <div className="flex items-center justify-between border-b border-slate-800/80 pb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="bg-emerald-950 text-emerald-400 border border-emerald-800 text-[10px] font-mono px-1.5 py-0.5 rounded font-bold">
                            STEP {idx + 1}
                          </span>
                          <span className="font-bold text-slate-200 text-xs">{step.title}</span>
                        </div>
                        <span className="text-[10px] bg-slate-900 border border-slate-800 px-2 py-0.5 rounded text-cyan-400 font-mono shrink-0">
                          {step.category}
                        </span>
                      </div>
                      ...
                      <div className="bg-slate-900 p-2.5 rounded-lg border border-slate-800 font-mono text-[11px] space-y-1.5">
                        <div className="flex items-start gap-1">
                          <span className="text-slate-500 shrink-0 w-20">Formula:</span>
                          <span className="text-cyan-300 font-bold">{step.formula}</span>
                        </div>
                        <div className="flex items-start gap-1">
                          <span className="text-slate-500 shrink-0 w-20">Substituted:</span>
                          <span className="text-amber-300">{step.substitutedValues}</span>
                        </div>
                        <div className="flex items-start gap-1 border-t border-slate-800/80 pt-1 mt-1">
                          <span className="text-slate-500 shrink-0 w-20">Calculated:</span>
                          <span className="text-emerald-400 font-extrabold">{step.result}</span>
                        </div>
                      </div>
                    </div>
                  ))}
